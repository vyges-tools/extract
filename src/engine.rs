...
/// Load the job's DEF + rules and extract lumped parasitics for every net.
pub fn extract(job: &ExtractJob) -> Result<Vec<NetParasitics>, ExtractError> {
...
pub fn run_to_spef(job: &ExtractJob) -> Result<String, ExtractError> {
    let parasitics = extract(job)?;
    Ok(spef::render(&job.design, &Units::default(), None, &parasitics))
}
